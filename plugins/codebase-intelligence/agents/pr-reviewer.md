...
description: >
  ...
  the merged-candidate diff against the target repo's .claude/ rules + conventions, one line per
...
Review the merged-candidate diff against (a) the target repo's `.claude/` + `CLAUDE.md`
MUST/SHOULD/MUST-NOT/SHOULD-NOT rules and (b) repo conventions. Try to **falsify** the claim that the
